...
final class DzWeekPayments
{
    ...
    public static function weeklyPaidSeries(
        ...
        // НЕ используем view= : вид «Оплачено CSM» может иметь встроенный фильтр по дате (например,
        // «текущий месяц»), из-за чего исторические записи не возвращаются. Вместо этого явно
        // фильтруем по статусу и дате — это даёт полный диапазон за 12 недель.
        $formula = "AND("
            . "NOT(OR({Статус оплаты}='Не оплачен',{Статус оплаты}='Оплачен частично')),"
            . "{Дата оплаты счета},"
            . "IS_AFTER({Дата оплаты счета},'{$from}')"
            . ")";
