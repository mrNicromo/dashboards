...
3. После `git pull` в корне лежит шаблон **`.env`** (без секретов). Чтобы всё заработало на сайте, добавьте на сервере **`/.env.local`** рядом с `.env` с реальными `AIRTABLE_PAT`, ключами AI и при необходимости `DASHBOARD_API_SECRET` — `dashboard/bootstrap.php` читает сначала `.env`, затем **перекрывает** значениями из `.env.local`. Либо задайте те же имена в панели хостинга (Railway, VPS `Environment=`, и т.д.).
...
## 4. Railway (бесплатный тариф и лишние расходы)

- **Не добавляйте PostgreSQL** (и другие БД) к этому сервису, если дашборд ими не пользуется: деньги за плагин + лимит `max_client_conn` при лишних подключениях. Данные — Airtable и файловый кэш в `dashboard/cache/`.
- Достаточно **одного** веб-сервиса из репозитория (Dockerfile → PHP). Не поднимайте второй сервис без нужды.
- Сборка использует **`.dockerignore`**: в образ не попадают `serve.mjs`, CSV из `tools/`, тяжёлые `.md` — меньше контекста и быстрее деплой.

## 5. Проверка живости (балансировщик)
